...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .register-page-wrapper { flex-grow: 1; display: flex; justify-content: center; align-items: center; padding-top: 120px; padding-bottom: 80px; width: 100%; }
        .register-form-container { width: 100%; max-width: 600px; padding: 20px; text-align: center; }
        .register-form-container h1 { font-size: 36px; margin-bottom: 30px; color: var(--asikzade-dark-text); font-weight: 600; }
        ...
        .input-group input[type="password"] {
            width: 100%; padding: 12px 15px; border: 1px solid var(--login-input-border); background-color: var(--login-input-bg); border-radius: 6px; font-size: 14px; color: var(--asikzade-dark-text);
        }
        ...
        .register-btn { background-color: var(--login-button-bg); color: var(--login-button-text); border: none; width: 100%; padding: 15px; border-radius: 50px; font-size: 16px; font-weight: 500; cursor: pointer; margin-top: 15px; transition: background-color 0.3s ease, transform 0.2s ease; text-transform: none; letter-spacing: normal; }
        .register-btn:hover { background-color: var(--login-button-hover-bg); transform: translateY(-2px); }
        .register-btn:active { transform: translateY(0); }
        .login-link { margin-top: 30px; font-size: 14px; color: var(--asikzade-gray); }
        .login-link a { color: var(--asikzade-green); font-weight: 500; text-decoration: none; }
        .login-link a:hover { text-decoration: underline; color: var(--asikzade-dark-green); }
        ...
        @media (max-width: 768px) {
            ...
            .register-page-wrapper { padding-top: 100px; padding-bottom: 60px; } .register-form-container { max-width: 500px; } .register-form-container h1 { font-size: 30px; margin-bottom: 30px; } .form-row { flex-direction: column; gap: 20px; }
        ...
        @media (max-width: 480px) {
            ...
            .register-page-wrapper { padding-top: 90px; padding-bottom: 40px; } .register-form-container { max-width: 100%; padding: 15px; } .register-form-container h1 { font-size: 26px; } .input-group input { padding: 11px 14px; font-size: 13px;} .register-btn { padding: 14px; font-size: 15px;} .form-row { gap: 15px; }
        }
    </style>
</head>
<body>
            ...
            <form action="register_process.php" method="POST" class="register-form">
                <div class="form-row">
                    <div class="input-group">
                        ...
                        <input type="text" id="ad" name="ad" placeholder="Adınız" required value="<?php echo htmlspecialchars($_SESSION['form_data']['ad'] ?? ''); ?>">
                    </div>
                    <div class="input-group">
                        <label for="soyad">Soyadınız</label>
                        <input type="text" id="soyad" name="soyad" placeholder="Soyadınız" required value="<?php echo htmlspecialchars($_SESSION['form_data']['soyad'] ?? ''); ?>">
                    </div>
                </div>

                <div class="input-group">
                    <label for="email">E-posta Adresiniz</label>
                    <input type="email" id="email" name="email" placeholder="ornek@eposta.com" required value="<?php echo htmlspecialchars($_SESSION['form_data']['email'] ?? ''); ?>">
                </div>
                ...
                <div class="input-group">
                    <label for="sifre_tekrar">Şifrenizi Tekrar Girin</label>
                    <input type="password" id="sifre_tekrar" name="sifre_tekrar" placeholder="Şifrenizi onaylayın" required>
                </div>
    ...
    <script>
        ...
        if (form && sifreInput && sifreTekrarInput) {
            form.addEventListener('submit', function(event) {
                if (sifreInput.value !== sifreTekrarInput.value) {
                    alert('Şifreler eşleşmiyor!');
                    sifreTekrarInput.focus();
                    event.preventDefault(); 
                    return false;
                }
                if (sifreInput.value.length < 6) { // Şifre uzunluğunu 6'ya düşürdüm
